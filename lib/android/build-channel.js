...
if (VERSION_FROM_GRADLE != PROJECT_VERSION) {
  throw new Error(`PROJECT_VERSION ${PROJECT_VERSION} does not match version from gradle build file ${VERSION_FROM_GRADLE}!`);
}
...
function buildChannel(flavor, channelFilePath) {
  ...
  const channels = fs.readFileSync(channelFilePath).toString().split(/\r\n|\n/).join(', ');
  util.logLine();
  console.log('Building channels for ' + flavor);
  console.log('Channels: ' + channels);
  const versionApkFile = path.join(apkDir, `${flavor}_${BUILD_TYPE}_${PROJECT_VERSION}.apk`);
  ...
  if (BUILD_TYPE == 'release') {
    ...
    const buildApkFile = path.join(apkDir, `${flavor}_${BUILD_TYPE}_${PROJECT_VERSION}_${BUILD_NUMBER}.apk`);
    ...
    console.log('Version APK: ' + versionApkFile);
    console.log('Latest APK: ' + latestApkFile);
    execSync(`java -jar node_modules/app-node-tool/lib/android/walle-cli-all.jar batch -f channel/${flavor}.txt ${buildApkFile} ${APK_DIST}`);
  }
  execSync(`java -jar node_modules/app-node-tool/lib/android/walle-cli-all.jar batch -f channel/${flavor}.txt ${versionApkFile} ${APK_DIST}`);
  execSync(`java -jar node_modules/app-node-tool/lib/android/walle-cli-all.jar batch -f channel/${flavor}.txt ${latestApkFile} ${APK_DIST}`);
